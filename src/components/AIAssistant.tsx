    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-[450px] h-[600px] flex flex-col glass-panel rounded-3xl overflow-hidden animate-in fade-in zoom-in duration-300 shadow-[0_0_50px_rgba(255,255,255,0.05)]">
            {/* Header */}
                ...
                >
                  <div className={`flex gap-2 max-w-[85%] ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                    <div className={`mt-1 h-6 w-6 rounded-lg flex items-center justify-center shrink-0 border border-white/10 ${
                      msg.sender === 'user' ? 'bg-white/10 text-white' : 'bg-white/10 text-white'
                    }`}>
                    ...
                    <div>
                      <div className={`p-3 rounded-2xl text-xs leading-relaxed ${
                        msg.sender === 'user'
                          ? 'bg-white/20 text-white border-br-none rounded-tr-none'
            ...
            >
              <div className="relative">
                <input
                  ...
                  className="w-full bg-black/40 border border-white/10 text-xs text-white rounded-xl py-3 pl-4 pr-12 focus:outline-none focus:border-white/50 transition-colors"
                />
                <button
                  type="submit"
                  disabled={!inputText.trim()}
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg bg-white text-black disabled:opacity-30 transition-opacity"
                >
                  <Send size={14} />
                </button>
              </div>
