...
class HardFilter:
    ...
    def is_valid(url: str) -> bool:
        try:
            ...
            if any(pattern in domain for pattern in retail_patterns): return False
            if domain in HardFilter.EXCLUDED_DOMAINS or any(domain.endswith("." + ex) for ex in HardFilter.EXCLUDED_DOMAINS): return False
            return "." in domain
...
class CompetitorScorer:
    ...
    def validate_relevance(content: str, industry: str, city: str, level: int = 1, profile: dict = None) -> bool:
        ...
        if profile and profile.get("anti_markers"):
            ...
            if profile.get("is_agency") and anti_hits >= 2: return False
            if anti_hits >= 5: return False

        from app.services.competitor_intelligence import competitor_service
        all_markers = competitor_service.get_type_markers()
        candidate_scores = {b_type: sum(1 for m in markers if m in content_lower) for b_type, markers in all_markers.items()}
        ...
        if profile and profile.get("type") != "GENERIC":
            ...
            if user_type in ["AGENCY", "RETAIL", "ECOMMERCE", "REAL_ESTATE", "EDUCATION", "HEALTHCARE"]:
                if candidate_type != user_type and candidate_scores.get(user_type, 0) < 2: return False
        ...
        for key, kws in industry_keywords.items():
            if key in industry_lower or any(kw in industry_lower for kw in kws): check_list.extend(kws)
        ...
        city_aliases = {"madhapur": ["hyderabad", "telangana", "hitech city"], "gachibowli": ["hyderabad", "telangana"], "kondapur": ["hyderabad", "telangana"], "whitefield": ["bangalore", "bengaluru"]}
        valid_locations = [city_lower] + city_aliases.get(city_lower, [])
...
class CompetitorIntelligenceService:
    ...
    def get_type_search_suffix(self, b_type: str) -> str:
        mapping = {"AGENCY": "Agency", "RETAIL": "Store", "ECOMMERCE": "Online Shop", "SAAS_TECH": "Software Platform", "REAL_ESTATE": "Company", "EDUCATION": "Institution", "HEALTHCARE": "Services", "FINANCE": "Services", "HOSPITALITY": "Hospitality", "MANUFACTURING": "Manufacturer", "PROFESSIONAL_SERVICES": "Consultancy", "MEDIA_PUBLISHER": "Publisher", "MARKETPLACE": "Marketplace", "DIRECTORY": "Directory"}
    ...
    async def profile_user_business(self, site_url: str):
        ...
        scores = {b_type: sum(1 for m in markers if m in content_lower) for b_type, markers in all_markers.items()}
        primary_type = max(scores, key=scores.get)
        if scores[primary_type] == 0: primary_type = "GENERIC"
        self.business_profile = {"type": primary_type, "scores": scores, "is_agency": primary_type == "AGENCY", "anti_markers": []}
        if primary_type == "AGENCY": self.business_profile["anti_markers"] = all_markers["RETAIL"] + all_markers["ECOMMERCE"] + all_markers["DIRECTORY"]
        elif primary_type in ["RETAIL", "ECOMMERCE"]: self.business_profile["anti_markers"] = all_markers["AGENCY"] + all_markers["DIRECTORY"]
    ...
    async def discover_candidates(self, services: List[str], city: str, force_type: str = None) -> List[Dict[str, Any]]:
        ...
        refined_services = [f"{s.strip()} {type_suffix}" if type_suffix and type_suffix.lower() not in s.lower() else s.strip() for s in services]
        ...
        scored = [{ "domain": d, "avg_position": sum(v["positions"])/len(v["positions"]), "occurrences": len(v["positions"]), "representative_url": v["urls"][0], "score": self.scorer.score_competitor(d, len(v["positions"]), sum(v["positions"])/len(v["positions"]), 70) } for d, v in raw_results.items()]
    ...
    def analyse_text(self, content: str) -> dict:
        ...
        found_cta = [cta for cta in ["book now", "contact us", "get quote", "enquire now", "call us"] if cta in content.lower()]
        trust_signals = [word for word in ["years of experience", "trusted", "award", "certified", "iso"] if word in content.lower()]
